...
class Fish():
    ...
    def draw(self):
        ...
        tail_animation_x = sin(frame_count*self.fish_speed/5)*self.fish_size[0]/40
        tail_animation_y = sin(frame_count*self.fish_speed/10)*self.fish_size[1]/20
    ...
    def move(self):
        ...
        if self.position[0]+move_x > width+70 or self.position[0]+move_x < -70:
            move_x = -move_x
            move_y /= 5
            self.rotation = atan2(move_y, move_x)
        if self.position[1]+move_y > height or self.position[1]+move_y < 0:
            move_y = -move_y
            self.rotation = atan2(move_y, move_x)
...
class Bubble:
    ...
    def rise(self):
        ...
        if self.position[1] < -50:
            self.position = [random(0, width), height+50]
...
class Seaweed():
    ...
    def draw(self):
        ...
        for i in range(res):
            ...
            weed_point1 = [self.position + sin((frame_count + self.position + i*res)*0.05)*(res-i),
                            height*0.4*(i/res) + height*0.5]
            weed_point2 = [self.position + sin((frame_count + self.position + (i+1)*res)*0.05)*(res-i+1),
                            height*0.4*((i+1)/res) + height*0.5]
...
def setup():
    ...
    for i in range(30):
        ...
        position 		= [random(0.1*width, 0.9*width), random(0.1*height, 0.9*height)]
    ...
    for i in range(25):
        bubble_pos = [random(0, width), random(0, height)]
...
def draw():
    ...
    for food in food_list:
        food.fall(0.098)
        food.draw()
        if food.position[1] > height or -50 > food.position[0] or width + 50 < food.position[0]:
    ...
    if is_mouse_pressed:
        ...
        food_list.append(Food([mouse_x+random(-20, 20), 1], food_velocity, food_size))